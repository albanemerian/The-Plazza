...
std::string Cooks::toString(PizzaType type) {
    switch (type) {
...
std::shared_ptr<IRecipe> Cooks::findAndLoadPlugin(const std::string &pizzaType,
    int number) {
    std::string pluginDir = "./plugins/";

    try {
        if (!std::filesystem::exists(pluginDir)) {
            std::cerr << "Plugin directory does not exist: " <<
                pluginDir << std::endl;
            return nullptr;
        }
            ...
            if (entry.is_regular_file() && entry.path().extension() == ".so") {
                ...
                try {
                    ...
                    if (pluginType == pizzaType) {
                        ...
                        FoodFunc createFood = reinterpret_cast<FoodFunc>
                            (createSymbol);
        ...
        std::cerr << "Error scanning plugin directory: " << e.what()
            << std::endl;
    }

    std::cerr << "Plugin for " << pizzaType << " not found in " <<
        pluginDir << std::endl;
    return nullptr;
}
