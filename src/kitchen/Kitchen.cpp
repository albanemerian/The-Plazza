...
void Kitchen::setCurrentOrders(int currentOrders) {
        ...
        for (auto &ing : status) {
            if (ing.quantity == 0) {
                while (ing.quantity < 5) {
                    ...
                    /* Log Output */
                    this->_logger->log("Restocking ingredient type: " +
                        std::to_string(ing.type) + " from " +
                        std::to_string(ing.quantity) + " to " +
                        std::to_string(ing.quantity + 1));
...
void Kitchen::startKitchenProcess() {
    ...
    try {
        ...
        /* Log Output */
        _logger->log("Created kitchen process with PID: " +
            std::to_string(_pid));
    } catch (const std::exception &e) {
...
void Kitchen::run() {
    ...
    while (_isRunning) {
        try {
            ...
            /* Log Output */
            _logger->log("Kitchen " + std::to_string(_ID) +
                " Duration: " + std::to_string(duration) +
                " CurrentOrders: " + std::to_string(_currentOrders) +
                " QueueSize: " + std::to_string(_orderQueue.size()));
    ...
    if (orderData.substr(0, 4) == "0x01") {
        try {
            ...
            if (auto order = std::dynamic_pointer_cast<Order>(unpackedMsg)) {
                ...
                /* Log Ouput */
                _logger->log("Added " + std::to_string(numPizzas) +
                    " pizzas to order queue, notifying cooks");
            }
        } catch (const std::exception &e) {
